...
void main()
{
	...
	for( i = 0; i < beamer_count; i++){
		...
		angle = (beamer_enable[i] == 10)?dot(jit_in.normal, raynormal) :dot(jit_in.normal, raynormal * (1.0 - angle_mode) + beamer_dir[i] * angle_mode);
        ...
        proj_texUV[i] = (beamer_enable[i] == 10)?vec4((theta + PI) / (2 * PI), 1.0 - phi / PI, 0., 1.): jit_in.beamer_uv[i];
    ...
    color = vec4(color.rgb, blendRef);
    
    outColor0 = alphablend(color, offColor);
}